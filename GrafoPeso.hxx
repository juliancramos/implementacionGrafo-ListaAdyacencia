...
template <typename T>
void GrafoPeso<T>::agregarArista(const T& nodo1, const T& nodo2, double peso) {
...
// Mostrar el GrafoPeso con pesos
template <typename T>
void GrafoPeso<T>::mostrarGrafoPeso() const {
...
// DFS recursivo
template <typename T>
void GrafoPeso<T>::dfs(const T& nodo, std::set<T>& visitados) const {
    visitados.insert(nodo);
    std::cout << nodo << " "; // Imprimir el nodo visitado

    //for (const auto& [vecino, _] : listaAdyacencia.at(nodo)) { //Para c++17, lo que hace es descartar el peso
...
template <typename T>
void GrafoPeso<T>::iniciarDFS(const T& nodoInicial) const {
...
// BFS (anchura)
template <typename T>
void GrafoPeso<T>::bfs(const T& nodoInicial) const {
    std::set<T> visitados; 
    ...
    while (!cola.empty()) {
        ...
        //for (const auto& [vecino, _] : listaAdyacencia.at(nodo)) { //Esto es para c++17
            for (const auto& vecino : listaAdyacencia.at(nodo)) {
            if (visitados.find(vecino) == visitados.end()) {
